...
// >> BiglyBT
        ...
        // ".button()" to give it jQuery UI style
        $('#upload_confirm_button, #upload_cancel_button, #rename_confirm_button, #rename_cancel_button, #upload_confirm_button, #upload_cancel_button, #move_confirm_button, #move_cancel_button').button();
// << BiglyBT
...
/* BiglyBT: Replaced transMenu with jQueryUI.menu
...
// >> BiglyBT: Pairing Logout, 'Add' for isMobileDevice (Why?)
...
// << BiglyBT
    ...
    createContextMenu: function () {
        ...
        // See https://github.com/mar10/jquery-ui-contextmenu/wiki/ApiRef#options
...
*/
        ...
        $("#settings_menu").click(function (event) {
            $("#footer_super_menu").toggle().focus();
/* BiglyBT: Replaced transMenu with jQueryUI.menu
...
// >> BiglyBT: Replaced transMenu with jQueryUI.menu.
        ...
        var elementMenuText = element.find('div');
...
*/
        if (element.hasClass('sort-mode')) {
            element.parent().children('.sort-mode.menu-item-selected').removeClass('menu-item-selected');
            element.addClass('menu-item-selected');
            this.setSortMethod(id.replace(/sort_by_/, ''));
        } else if (element.hasClass('upload-speed')) {
            o = {};
            o[RPC._UpSpeedLimit] = parseInt(elementMenuText.html());
            o[RPC._UpSpeedLimited] = true;
            remote.savePrefs(o);
        } else if (element.hasClass('download-speed')) {
            o = {};
            o[RPC._DownSpeedLimit] = parseInt(elementMenuText.html());
            o[RPC._DownSpeedLimited] = true;
            remote.savePrefs(o);
        } else {
            switch (id) {
// BiglyBT: Add compact view and prefernces menus back in
            case 'compact_view':
                this.toggleCompactClicked();
                break;
...
// << BiglyBT
            ...
            case 'homepage':
                window.open('http://www.biglybt.com/');
                break;

            case 'tipjar':
                window.open('https://www.transmissionbt.com/donate/?sourceRef=BiglyBT');
                break;
...
// << BiglyBT
    ...
    promptToRenameTorrent: function (torrent) {
        ...
        $(document).scrollTop($('#rename_container').offset());
    ...
    updateGuiFromSession: function (o) {
        ...
        */
        ...
        if (this.isMenuEnabled && (RPC._DownSpeedLimited in o) && (RPC._DownSpeedLimit in o)) {
            ...
            /* BiglyBT: Replaced transMenu with jQueryUI.menu, which has text in <div>
            e.html('Limit (' + fmt.speed(limit) + ')');
            */
            e.find("> div").html('Limit (' + fmt.speed(limit) + ')');

            if (!limited) {
                e = menu.find('#unlimited_download_rate');
            };

            /* BiglyBT: Replaced transMenu with jQueryUI.menu, which doesn't have a selectMenuItem(), so we do it manually
            e.selectMenuItem();
            */
// >> BiglyBT: Unselect all, and select correct
            e.parent().children('.menu-item-selected').removeClass('menu-item-selected');
...
// << BiglyBT
    ...
    */
    ...
    setFilterMode: function (mode) {
        ...
        // >> Vuze: Make sure filter mode widget has correct value
